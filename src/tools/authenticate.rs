...
//! <https://open.longportapp.com/connect>, generates a short-lived, one-time
...
//! LongPort token endpoint and returns the resulting access token to the
...
pub const CONNECT_PAGE_URL: &str = "https://open.longportapp.com/connect";

/// `/connect` page URL matching the deployment region: a `.cn` deployment
/// (detected via `LONGPORT_HTTP_URL` pointing at openapi.longportapp.cn)
/// should send users to open.longportapp.cn instead of the global site.
pub fn connect_page_url() -> &'static str {
    if oauth_base_url().contains("longportapp.cn") {
        "https://open.longportapp.cn/connect"
    } else {
        CONNECT_PAGE_URL
    }
}
...
/// deployment binds to open.longportapp.cn, everything else to open.longportapp.com.
/// Must stay in sync with the web `getAgentRedirectUri()` and the CLI.
fn agent_redirect_uri() -> &'static str {
    if oauth_base_url().contains("longportapp.cn") {
        "https://open.longportapp.cn/connect/done"
    } else {
        "https://open.longportapp.com/connect/done"
    }
}
...
pub struct AuthenticateParam {
    ...
    /// <https://open.longportapp.com/connect>. Valid for 10 minutes and usable
    /// only once.
...
/// Base OAuth URL (no trailing slash), honoring `LONGPORT_HTTP_URL` so tests
/// and non-prod deployments can redirect the exchange. Mirrors
/// `auth::metadata::longport_oauth_url`.
...
pub async fn authenticate(
    ...
    let Some((client_id, real_code)) = unpack_agent_code(&param.auth_code) else {
        return Err(McpError::invalid_params(
            format!(
                "`auth_code` is not a valid LongPort authorization code. {}",
                self_heal_hint()
            ),
            None,
        ));
    ...
    let next_steps = format!(
        "Authentication successful. The full tool set unlocks once requests carry this access token. \
         ...
         `claude mcp add --transport http longport https://mcp.longportapp.com --header \"Authorization: Bearer {token}\"`. \
...
async fn exchange_code(code: &str, client_id: &str) -> Result<TokenResponse, McpError> {
    ...
    let client = reqwest::Client::builder()
        ...
        .user_agent(concat!("longport-mcp/", env!("CARGO_PKG_VERSION")))
        .build()
    ...
    let resp = client.post(&url).form(&form).send().await.map_err(|e| {
        McpError::internal_error(
            format!("failed to reach the LongPort token endpoint: {e}"),
            None,
        )
    })?;
...
mod tests {
    ...
    async fn exchange_failure_includes_self_heal_hint() {
        ...
        // Serialized against other tests that mutate LONGPORT_HTTP_URL. The
