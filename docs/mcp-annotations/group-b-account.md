...
trade-record query tools in the LongPort MCP server, for the OpenAI MCP
...
- **Open World = true** — It reaches the external LongPort brokerage backend over the network. Balances reflect live market valuations and account activity outside this server's control, so successive reads may legitimately return different values.
...
- **Open World = true** — Data comes from the remote LongPort account backend and changes as fills/corporate actions occur outside this server.
...
- **Open World = true** — Backed by the external LongPort backend; NAV and holding units update over time independently of this server.
...
- **Open World = true** — Margin factors are served by the remote LongPort backend and may change per upstream risk policy, outside this server's control.
...
- **Open World = true** — Reads live order state from the LongPort backend, which changes as orders are submitted/filled elsewhere.
...
- **Open World = true** — Execution data is fetched live from the LongPort backend and grows as trades fill during the day.
...
- **Open World = true** — Historical order data resides on the remote LongPort backend and is not controlled by this server.
...
- **Open World = true** — Backed by the external LongPort backend serving real historical trade records.
...
- **Open World = true** — Records come from the remote LongPort backend and reflect account activity outside this server's control.
...
- **Open World = true** — The estimate is computed by the remote LongPort backend using live buying power and margin data, which vary over time outside this server.
...
- **Open World = true** — Statement metadata is served by the remote LongPort backend, where new statements appear over time independently of this server.
...
- **Open World = true** — The URL is minted by the remote LongPort backend / object store, outside this server's control; the link is time-limited and externally managed.
...
- **Open World = true** — Card data is served by the remote LongPort `/v1/account/bank-cards` endpoint, outside this server's control.
...
- **Open World = true** — History is served by the remote LongPort backend and grows as real withdrawals occur outside this server.
...
- **Open World = true** — History is served by the remote LongPort backend and reflects real deposit activity outside this server's control.
