...
- **openWorldHint = true** — the tool reaches out over the network to the
  external LongPort backend, whose data (news feeds, community posts, screener
...
- **Open World = true** — News content comes from the external LongPort backend and updates continuously as new articles are published.
...
- **Open World = true** — Results are served by the external LongPort search backend and shift as news is indexed.
...
- **Open World = true** — Community topics live on the external LongPort backend and are updated by other users in real time.
...
- **Open World = true** — Topic data is hosted on the external LongPort backend and may change as the post is edited or gains likes/comments.
...
- **Open World = true** — Replies are stored on the external LongPort backend and grow as users respond.
...
- **Open World = true** — Topic search results come from the external LongPort backend and vary as content is indexed.
...
- **Open World = true** — Strategy presets and their `three_months_chg` figures are served by the external LongPort backend and updated server-side.
...
- **Open World = true** — The saved list is account-scoped data held on the external LongPort backend.
...
- **Open World = true** — Strategy definitions are stored on the external LongPort backend.
...
- **Open World = true** — The screen runs against the live LongPort universe and indicator values on the external backend, which change with the market.
...
- **Open World = true** — Indicator metadata is supplied by the external LongPort backend.
...
- **Open World = true** — Leaderboard category config is served by the external LongPort backend.
...
- **Open World = true** — Rankings are computed live on the external LongPort backend and change with market activity (`updated_at` is returned).
...
- **Open World = true** — The subscription pipeline is maintained on the external LongPort backend and changes as IPOs open/close.
...
- **Open World = true** — The IPO calendar is served by the external LongPort backend and updated as schedules change.
...
- **Open World = true** — Listing data and first-day returns come from the external LongPort backend.
...
- **Open World = true** — IPO profile/timeline/eligibility data is hosted on the external LongPort backend.
...
- **Open World = true** — IPO orders are account-scoped records on the external LongPort backend; their status changes server-side (e.g. allotment).
...
- **Open World = true** — The order record lives on the external LongPort backend and its status may update server-side.
...
- **Open World = true** — P/L is derived server-side on the external LongPort backend from live valuations.
...
- **Open World = true** — DCA plans are account-scoped records on the external LongPort backend, advanced server-side on schedule.
...
- **Open World = true** — Execution history is stored on the external LongPort backend and grows as the plan executes.
...
- **Open World = true** — Statistics are computed server-side on the external LongPort backend from live valuations.
...
- **Open World = true** — Support flags are determined by the external LongPort backend (per-instrument eligibility rules).
...
- **Open World = true** — Sharelists are community data on the external LongPort backend, changed by their owners/followers.
...
- **Open World = true** — Sharelist content (constituents, live quotes, subscription status) is served by the external LongPort backend.
...
- **Open World = true** — Popularity rankings are computed on the external LongPort backend and shift with community activity.
...
- **Open World = true** — Alerts are account-scoped records on the external LongPort backend; their `triggered_at`/`enabled` state updates server-side.
...
- **Open World = true** — The computation depends on live/historical K-line data served by the external LongPort backend, which is not under this server's control.
